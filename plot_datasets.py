...
ax = sns.heatmap(znew_nn)

points=np.zeros((len(x),2))
points[:,0]=x
points[:,1]=y

interplin = LinearNDInterpolator(points, z, fill_value=0)
zlin=interplin(xx, yy)
    
znew_lin=zlin.reshape(50,50)     
         
sns.heatmap(znew_lin)

fcubic = interp2d(x, y, z, kind='cubic')  #spline
zcubic=fcubic(xnew, ynew)
 
znew_cubic=zcubic.reshape(50,50)     
sns.heatmap(znew_cubic)
